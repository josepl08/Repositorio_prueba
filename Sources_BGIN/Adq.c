...
void Lecturas_fin(void){
	...
	/*for( med=0 ; med < 16; med++ )
		for( col=0 ; col < 8; col++ )
			for( row = 15 ; row != 0xFF ; row-- ){
				Data[col][med] 		|= ((int)Array[med][row] 	& (1<<col)) << (int)(15-row-col);
				Data[col+8][med] 	|= ((int)Array[med][row+16] & (1<<col)) << (int)(15-row-col);
			}*/
